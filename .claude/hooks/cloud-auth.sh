...
# Ensure gcloud tools (gcloud, bq, gsutil) are on PATH for this and future shells
GCLOUD_BIN="/home/user/google-cloud-sdk/bin"
if [ -d "$GCLOUD_BIN" ]; then
  export PATH="$GCLOUD_BIN:$PATH"
  PROFILE="${HOME}/.bashrc"
  if ! grep -q "google-cloud-sdk/bin" "$PROFILE" 2>/dev/null; then
    echo "export PATH=\"$GCLOUD_BIN:\$PATH\"" >> "$PROFILE"
  fi
fi
