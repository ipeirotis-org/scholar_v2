...
def results():
    ...
    if author.get("publications"):
         ...
         df.dropna(subset=["pub_year"], inplace=True) # Drop rows where pub_year couldn't be converted
    ...
    if author.get("temporal_stats"):
        ...
        temporal_df['state_year'] = pd.to_numeric(temporal_df['state_year'], errors='coerce')
        temporal_df.dropna(subset=['state_year'], inplace=True)

        if not temporal_df.empty:
             temporal_plots['h_index'] = generate_author_h_index_plot(temporal_df)
             temporal_plots['total_citations'] = generate_author_total_citations_plot(temporal_df)
    ...
    return render_template("results.html", author=author, plot1=plot1, plot2=plot2, temporal_plots=temporal_plots)
