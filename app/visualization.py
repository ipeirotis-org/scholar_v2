...
def _generate_temporal_plot(temporal_df, y_value_col, y_perc_col, title, y_value_label, y_perc_label="Percentile"):
    """Helper function to generate a dual-axis temporal plot."""
    if temporal_df.empty or y_value_col not in temporal_df.columns or y_perc_col not in temporal_df.columns:
    ...
    try:
        ...
        ax1.plot(temporal_df['state_year'], temporal_df[y_value_col], color=color1, marker='o', label=y_value_label)
        ...
        ax2.plot(temporal_df['state_year'], temporal_df[y_perc_col] * 100, color=color2, marker='x', linestyle='--', label=y_perc_label) # Multiply percentile by 100
        ...
        ax1.legend(lines + lines2, labels + labels2, loc='upper center', bbox_to_anchor=(0.5, -0.15), fancybox=True, shadow=True, ncol=2)
